...
docker run --rm -v "$PWD":"/go/" -w "/go/src" iron/go:dev go build -o ../dist/solar-default-controller tsai.eu/solar/controller/defaultController

# copy Dockerfile
cp docker/defaultController/Dockerfile dist

# change to "dist" directory
cd dist

# build docker image
docker build -q -t tsai/solar-default-controller:V1.0.0 .
