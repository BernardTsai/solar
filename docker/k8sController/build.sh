...
docker run --rm -v "$PWD":"/go/" -w "/go/src" iron/go:dev go build -o ../dist/solar-k8s-controller tsai.eu/solar/controller/k8sController

# copy Dockerfile
cp docker/k8sController/Dockerfile dist

# change to "dist" directory
cd dist

# build docker image
docker build -q -t tsai/solar-k8s-controller:V1.0.0 .
